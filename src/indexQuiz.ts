...
async function main() {
  ...
  async function comp(id: string, props: Record<string, any>) {
    return selectComposition({ serveUrl, id, inputProps: props, timeoutInMilliseconds: 120000 });
  }
  ...
  for (const lang of ['en', 'sk'] as const) {
    ...
    await renderMedia({ composition: q1, serveUrl, timeoutInMilliseconds: 120000, codec: 'h264', outputLocation: p1, inputProps: baseProps });
    ...
    await renderMedia({ composition: q2, serveUrl, timeoutInMilliseconds: 120000, codec: 'h264', outputLocation: p2, inputProps: baseProps });
    ...
    await renderStill({  composition: q3, serveUrl, timeoutInMilliseconds: 120000, output: p3, inputProps: { ...baseProps, explanation } });

    // Slide 4: CTA
    console.log(`🖼️ [${lang}] CTA slide`);
    const q4 = await comp('SlideCTA', { lang, equipment: {} });
    const p4 = path.join(OUT_DIR, `${lang}_quiz4.png`);
    await renderStill({  composition: q4, serveUrl, timeoutInMilliseconds: 120000, output: p4, inputProps: { lang, equipment: {} } });
