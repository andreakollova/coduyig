...
async function main() {
  ...
  for (const lang of langs) {
    ...
    // Delete old reels for same topic+lang (from IG + from Supabase Storage)
    try {
      ...
      mediaTrack.push({ topicId: topic.id, lang, mediaId: published.id, storagePath, publishedAt: new Date().toISOString() });
