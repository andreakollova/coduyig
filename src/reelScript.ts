...
2. TEACHER: Start with a SHORT intro sentence ending with a period: "Today we are looking at lambda functions." Then explain what it is in a separate sentence. Example: "Today we are looking at lambda functions. These are tiny shortcuts that let you write a function in one quick line instead of defining a whole named function." (max 45 words). NEVER read code. The first sentence MUST be short and end with a period before the explanation begins.
...
5. STUDENT: confirms understanding in ONE short sentence (max 15 words). Say it simply in their OWN words — DO NOT repeat the teacher's explanation back. Just capture the core idea in a fresh way. NEVER parrot the teacher's words or structure. Examples: "Oh so it's basically a shortcut for quick throwaway functions, got it." or "Right, so I just filter what I need without writing a whole loop." or "Makes sense, so it keeps everything organized in one place."
