...
2. TEACHER: Start with "Today we are looking at [topic]." (short sentence, period). Then explain WHAT it does and WHY you would use it, in plain words. Example: "Today we are looking at lambda functions. They let you write a quick one-line function when you don't need a full named one, which saves you time on small tasks." (max 45 words). NEVER read code. Explain it so a beginner understands on first listen.
...
5. STUDENT: gives a BEGINNER-FRIENDLY example of how they would use it in their own project (2-3 sentences, max 30 words). Pick a simple relatable scenario a beginner would build — a todo app, a simple game, a school project, a personal website. Walk through the example briefly. Examples: "So if I am building a todo app, I could use this to combine my work tasks and personal tasks into one list without any duplicates, right?" or "Nice, so in my school project I could take two class lists and quickly see which students are in both classes." ALWAYS a specific beginner scenario, NEVER restating what the teacher said.
