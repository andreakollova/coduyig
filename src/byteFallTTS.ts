...
const SK_PHONETICS: Record<string, string> = {
  ...
  'SMTP': 'es em tí pí',
  'IMAP': 'aj mep',
  // English words that SK TTS mispronounces
  'Apple': 'epl',
  'Clang': 'kleng',
  'LLVM': 'el el ví em',
  'Google': 'gúgl',
  'React': 'ríekt',
  'Python': 'pajton',
  'JavaScript': 'džavaskript',
  'TypeScript': 'tajpskript',
  'Node': 'noud',
  'Docker': 'doker',
...
export async function generateByteFallVoiceover(
  ...
  for (let i = 0; i < script.length; i++) {
    ...
    // Normalize audio — EN abbreviation lines are very quiet from ElevenLabs, SK is fine
    const wordCount = line.split(/\s+/).length;
    const isTermLine = i === 1 || i === script.length - 2; // "RL! Reinforcement Learning." lines
    const isShort = wordCount < 6;
    const boost = (isTermLine && lang === 'en') ? 'volume=3.5,' : isShort ? 'volume=2.0,' : '';
    try {
